...
df_prevalence_by_region <- df_qof_prevalence %>% 
  ...
  mutate(prevalence = register / list_size,
         prevalence_print = scales::percent(prevalence, accuracy = 0.01))
...
df_prevalence_by_region_table <- df_prevalence_by_region %>% 
    ...
    select(-c(patient_list_type, patient_list_type_num, patient_list_type_txt, prevalence))
```
...
reactable(df_prevalence_by_region_table,
          ...
          columns = list(group_code = colDef(name = "Clinical Area"),
                         ...
                         register = colDef(name = "Register"),
                         list_size = colDef(name = "List size"),
                         prevalence_print = colDef(name = "Prevalence"),
                         patient_list_type_wording = colDef(name = "List size type")))
```
